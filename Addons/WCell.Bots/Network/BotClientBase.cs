...
namespace WCell.Bots.Network
{
    ...
    public abstract class BotClientBase<C, PI, PO, PM> : IClient
        where C : IClient
        where PI : PacketIn
        where PO : PacketOut
        where PM : PacketManager<C, PI, BotPacketHandlerAttribute>
    {
        ...
        protected BotClientBase(ServerBase server, PM packetManager)
        {
            m_server = server;
        ...
        {
            throw new NotImplementedException("BotClientBase cannot receive asynchronously.");
        }

        public virtual void Connect(string host, int port)
        {
            throw new NotImplementedException("BotClientBase cannot connect anywhere.");
        }
